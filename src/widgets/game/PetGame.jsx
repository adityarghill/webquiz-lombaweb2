...
// ─────────────────────────────────────────────────────────────────────────────
export default function PetGame({ initialState = null }) {
  ...
  const handleSelectPet = (idx) => {
    ...
    setPetIndex(idx);
    setPetAnim("idle");
    // Simpan ke Supabase via window callback (injected oleh PetGameWithAuth)
    if (window.__petSaveCallback) {
      window.__petSaveCallback({ petIndex: idx, boredom, petBg });
    }
  };

  // ── Auto-save boredom & petBg setiap 30 detik ──────────────────────────────
  useEffect(() => {
    const timer = setInterval(() => {
      if (window.__petSaveCallback) {
        window.__petSaveCallback({ petIndex, boredom, petBg });
      }
    }, 30000);
    return () => clearInterval(timer);
  }, [petIndex, boredom, petBg]);
